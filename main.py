...
def print_match_result(match_result: MatchResult) -> None:
    ...
    matched_widths = (
        max([len(matched_headers[0]), *[len(row[0]) for row in matched_rows]], default=len(matched_headers[0])),
        max([len(matched_headers[1]), *[len(row[1]) for row in matched_rows]], default=len(matched_headers[1])),
    )
    ...
    if matched_rows:
        for recording_name, course_name in matched_rows:
            print(f'{recording_name:<{matched_widths[0]}} | {course_name:<{matched_widths[1]}}')
    else:
        print('No matched recordings.')

    print_recording_list('Unmatched recordings', match_result.unmatched)
    print_recording_list('Unmatched recordings with no candidate', match_result.unmatched_no_candidate)
...
def run_integration() -> None:
    ...
    if not args.auto_accept_prompts:
        change_settings_input = input('Do you wish to update settings for matched recordings? (Y/n): ')
