...
def _is_within_early_tolerance(
    session_start: datetime, recording_start: datetime, early_tolerance: timedelta
) -> bool:
    return session_start - early_tolerance <= recording_start


def _is_within_late_tolerance(
    session_start: datetime, recording_start: datetime, late_tolerance: timedelta
) -> bool:
...
def _get_session_recording_candidates(
    sessions_by_type: dict[str, list[CourseSession]],
    ...
    for session_type in sessions_by_type.keys():
        ...
        for session in sessions:
            while i < len(recordings):
                if not _is_within_late_tolerance(
                    session.scheduled_start_utc,
                    recordings[i].recording_start_utc,
                    late_tolerance,
                ):
                    break

                if _is_within_early_tolerance(
                    session.scheduled_start_utc,
                    recordings[i].recording_start_utc,
                    early_tolerance,
                ):
...
def _get_matches_from_candidates(
    recording_candidates: dict[str, list[Recording]],
) -> dict[str, Recording]:
...
def _prepare_match_result(
    ...
    candidate_counter = Counter(
        r.vimeo_video.uri for lst in candidates.values() for r in lst
    )
...
def match_session_recordings(
    sessions: list[CourseSession], recordings: list[Recording], settings: Settings
) -> MatchResult:
    sessions_by_type = get_sessions_by_type(sessions)
    recordings_by_type = get_recordings_by_type(recordings)
    candidates = _get_session_recording_candidates(
        sessions_by_type,
        recordings_by_type,
